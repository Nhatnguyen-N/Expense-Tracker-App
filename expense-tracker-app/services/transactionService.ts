...
const updateWalletForNewTransaction = async (
  walletId: string, amount: number, type: string
) => {
  try {
...
const revertAndUpdateWallets = async (
  ...
  try {
    ...
    // refetch the new walelt because we may have just updated it
...
export const deleteTransaction = async (transactionId: string, walletId: string): Promise<ResponseType> => {
  ...
  try {
    ...
    const newWalletAmount =
      walletData.amount! - (transactionType === "income"
        ? transactionAmount
        : -transactionAmount)
...
export const fetchWeeklyStats = async (uid: string): Promise<ResponseType> => {
  ...
  try {
    ...
    console.log("Error fetchign weekly stats:", error);
    return { success: false, msg: error.message }
  }
}
...
export const fetchMonthlyStats = async (uid: string): Promise<ResponseType> => {
  ...
  try {
    ...
    // Process transactios to calculate income and expense for each month
    querySnapshot.forEach((doc) => {
      const transaction = doc.data() as TransactionType;
    ...
    console.log("Error fetchign monthly stats:", error);
    return { success: false, msg: error.message }
  }
}
...
export const fetchYearlyStats = async (uid: string): Promise<ResponseType> => {
  ...
  try {
    ...
    console.log("Error fetchign yearly stats:", error);
    return { success: false, msg: error.message }
  }
}
